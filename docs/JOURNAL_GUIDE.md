...
现有一篇文章尚缺少 12 张原图，文件名和补传路径见 [待补图片清单](MISSING_IMAGES.md)。新出现的错误图片路径会在构建检查中报出。
...
文章固定使用居中的宽版正文，小屏自动适配，不需要填写排版字段。正文 `##`、`###` 标题会自动加入目录，每篇默认展开。正文样式集中在 `assets/css/journal.css`。
