...
- opening 页头沿用原 C 方案的两栏结构：左标题、右简介与元信息、底部横线；手机依次排列。可选的 `opening` 照片在横线下方按单图模块展示，编号与内容顺序保留。
...
- `Build and deploy OPTICS` 在 GitHub Pages Source 为 GitHub Actions 后负责发布；切换前只验证并生成构建产物，避免与默认分支发布相互覆盖。指南保留一次性启用步骤。

### 已提出、后续分步处理的视觉调整

1. 删除照片图注旁编号，加大摄影集不同模块之间的纵向间距。
2. 统一文章列表与摄影列表的抬头：橙色小字、顶部留白、分割线长度和颜色、简介位置。以实际两页中的“这里展示……”为定位依据，实施前核对页面，不混同详情页标题。
3. 摄影集列表先出多个现代风格方案，自动组合封面照片与标题；深浅两类由写作者选择，桌面网格每行两到三个。用户选定后再替换现有书册设计，未选定前不得写成定稿。
