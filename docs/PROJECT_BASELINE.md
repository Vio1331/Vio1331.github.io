...
按用户要求补齐首页影像板块：保留「02 / 影像」章节小字，右侧提供查看全部入口，下方直接复用已确认的刊物封面组件。自动展示日期最新的三个摄影集，随内容更新，不再依赖 `featured: true`。

封面标题、照片、日期、地点与深浅色读取摄影集自己的字段。桌面三列，800px 以下两列，600px 以下一列；使用现有页面宽度、纸色背景和响应式间距。原有首页开场、文章区域文案和文末文字不改写。

旧拟物书册不再挂载到首页；历史样式与资产保留。本轮首页组合布局尚未额外登记为用户已确认的视觉母版。
