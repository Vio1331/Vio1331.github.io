...
1. 统一文章列表与摄影列表的抬头：橙色小字、顶部留白、分割线长度和颜色、简介位置。以实际两页中的“这里展示……”为定位依据，实施前核对页面，不混同详情页标题。
2. 摄影集列表先出多个现代风格方案，自动组合封面照片与标题；深浅两类由写作者选择，桌面网格每行两到三个。用户选定后再替换现有书册设计，未选定前不得写成定稿。
