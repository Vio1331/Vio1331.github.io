...
两张方图的整体宽度与其他照片模块对齐，自动使用 M02-C 的较大间距；双联两侧不再额外内缩。横竖混合会等高排列。手机保留竖竖、方方双联并排；横横与混合比例默认上下排列。

如果某一组横图或混合比例双联在手机上也必须并排，可在该模块加 `mobile: pair`；想让任意双联在手机上上下排列，则加 `mobile: stack`。

完整的手机适配规则和封面副本目录见 [Markdown 摄影指南](PHOTOGRAPHY_GUIDE.md)。
...
`main: left` 是左图为主，改成 `right` 则右图为主。左右位置仍按照 `images` 的顺序。桌面上两张照片的垂直中心对齐，图注换行不会带偏照片位置。手机按文件顺序上下排列，主图整栏宽，小图为 80% 宽并居中。
