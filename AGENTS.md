...
- 首页：保留大标题与整屏章节节奏；文章主卡片在左、装饰「文」在右，摄影集完整封面在右、装饰「圖」在左。按日期倒序展示一个摄影主封面和两条次条，不复用摄影列表封面。
- 文章列表页：文章以卡片形式排列。
- 文章详情页：使用居中的正文版式。
...
- 装饰字为「文」「圖」，均使用 Dela Gothic One 原始字形的 SVG 线条轮廓，动画结束不淡出、不填充。两字各向页面外侧偏移，位置、字号和时间参数见基线末节。Freeman 继续使用 OPTICS 子集，正文、标题字体与品牌字距保持不变。
