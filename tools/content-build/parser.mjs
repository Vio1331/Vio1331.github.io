...
const fail = (env, line, message) => {
  throw new PhotographyError(message, env.file, env.offset + line + 1);
};
const markdown = new MarkdownIt({ html: true });
markdown.block.ruler.before('fence', 'photography_module', (state, start, end, silent) => {
  ...
  const token = state.push('photography_module', '', 0);
...
function units(source, env) {
  ...
  for (const token of tokens) {
    ...
    if (token.type === 'photography_module') {
...
export function parsePhotography(source, { file = '摄影集.md', offset = 0 } = {}) {
...
export function compileFile(input, file = '摄影集.md') {
  ...
  const match = /^---[ \t]*\n([\s\S]*?)\n---[ \t]*(?:\n|$)/.exec(source);
  if (!match) throw new PhotographyError('摄影集开头需要一对 ---，中间填写标题等信息。', file, 1);
  ...
  if (/^blocks\s*:/m.test(header)) {
    ...
    if (!parsed.errors.length && units(body, { file, offset }).some(item => item.kind === 'module')) throw new PhotographyError('同一摄影集不能同时使用 YAML blocks 和正文模块标记。', file, offset + 1);
    return { output: input, changed: false, legacy: true, warning };
  }
  const parsed = parseDocument(header);
  if (parsed.errors.length) throw new PhotographyError(`文件开头格式有误：${parsed.errors[0].message.split('\n')[0]}`, file, (parsed.errors[0].linePos?.[0]?.line || 1) + 1);
  const data = parsed.toJS();
  if (!data || typeof data !== 'object' || Array.isArray(data)) throw new PhotographyError('文件开头必须是 title: 等键值信息。', file, 2);
  const blocks = parsePhotography(body, { file, offset });
