...
import { compileFile, AlbumError } from './parser.mjs';
...
output ||= path.join(root, '_album_build');
const markerName = '.optics-album-build';
const markerValue = 'Disposable build copy created by tools/album-markup/cli.mjs\n';
...
async function main() {
  ...
  if (errors.length) {
    for (const error of errors) {
      ...
      if (process.env.GITHUB_ACTIONS && error instanceof AlbumError) {
  ...
  if (command === 'prepare') {
    ...
    const ignored = new Set(['.git', '.github', '.openai', 'node_modules', 'vendor', '_site', '_album_build', 'tools', 'validation']);
